<template>
    <section class=" w-[40rem] ">
            ...
            <div class="flex justify-center">
                ...
                <SelectMolecule v-model="data.country" :options="countries" placeholder="select country" >where do you live</SelectMolecule>
                <RadioMolecule  v-model="data.package" :options="packages" name="option"/>
            </div>
           ...
           <div class="py-8 px-4 bg-slate-100 flex justify-center">
            <div class="w-[20rem]">
                ...
                <div class="space-x-5">
                    ...
                    <TheButton v-step="'summary'" :disabled="data.name.length < 1 || data.age < 1">Next</TheButton>
                </div>
            </div>
           </div>
...
const packages = computed<SelectionType<packageType>[]>(() => {
    ...
    {value:"Safe",label:`Safe (+${ammount('Safe') - ammount('Standard') + ' ' + currency.value} , ${discounts['Safe']}% )`},
    {value:"Super Safe",label:`Super Safe (+${ammount('Super Safe') - ammount('Standard') + ' ' + currency.value} , ${discounts['Super Safe']}% )`},
    ])
